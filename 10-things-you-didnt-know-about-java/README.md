...
呃，你是不是学习写`Java`已经有些年头了？还依稀记得这些吧：
曾经，它还叫做`Oak`；曾经，`OO`还是个热门话题；曾经，`C++`同学们觉得`Java`是没有出路的；曾经，`Applet`还风头正劲……

但我打赌下面的这些事中至少有一半你还不知道。这周我们来聊聊这些会让你感到有些惊讶的`Java`内部的那些事儿吧。
...
你是不是想要继续了解语言规范和内核看看？可以在[这里](http://stackoverflow.com/q/442026/521799)找到更多有意思的细节。
...
是不是觉得这个很傻？想象一下在上面的代码中使用[`JSR-308`/`Java` 8的类型注解](https://jcp.org/en/jsr/detail?id=308)。
语法糖的数目要爆炸了吧！
